...
# Symlink analyze_brand_voice if not already linked
if [ ! -L "web/modules/contrib/analyze_brand_voice" ]; then
  ln -s /src web/modules/contrib/analyze_brand_voice
fi
...
# Run drupal-check
./vendor/bin/drupal-check --drupal-root . -ad web/modules/contrib/analyze_brand_voice 